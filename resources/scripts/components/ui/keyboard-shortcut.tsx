...
export const KeyboardShortcut: React.FC<KeyboardShortcutProps> = ({ keys, className, variant = 'default' }) => {
    const [platform, setPlatform] = useState<'mac' | 'pc'>('pc');
    const textColor = variant === 'faded' ? 'text-cream-400/30' : 'text-cream-400';
    ...
    const renderKey = (key: string) => {
        if (key === 'cmd' || key === 'ctrl') {
            return platform === 'mac' ? <CommandIcon /> : <span className={`text-xs ${textColor}`}>Ctrl</span>;
        }
    ...
    return (
        <div className={cn('inline-flex items-center gap-1 text-sm font-semibold select-none', className)}>
            {keys.map((key, index) => (
